import { setLocalStorage, remLocalStorage } from './localStorage.js';

const cart = document.getElementById('cart-count');

const setCartIconMain = (count) => {
  ...
  for (let button of collection) {
    ...
    button.addEventListener('click', (event) => {
      ...
      if (event.target.classList.contains('btn_in-cart')) {
        remLocalStorage(targetBook, 'id');
        deleteFromCart(event.target, localStorage.length);
      } else {
        setLocalStorage(bookInfo);
        addInCartNew(event.target, localStorage.length);
      }
    });
  }
};

const addInCartNew = (target, count) => {
...
export { getSelectedBookInfo, setCartIconMain };
